...
$.getJSON("data/iframe.json", function(json){

	var data = json.data;
	data.push({
    "dateOfIncident": (isMobile ? "Tap" : "Click") + " for more",
    "headline": "The Hindustan Times has catalogued " + json.meta.length + " hate crimes since September 28, 2015.",
    "url": ""
	})

	$(".counter .total").html(json.meta.length)
	// cards
	...
	$(".incident-block").draggable({
		...
		stop: function(){
			...
			if (isMobile){
				...
				setTimeout(function(){
					$(".incident-block-" + curr).animate({"left": default_left});	
				}, 10);	
			}
    ...
    swipe:function(event, direction, distance, duration, fingerCount, fingerData) {
    	...
    	if (options.curr_slide_number != data.length - 1 && dir == "next" || options.curr_slide_number != 0 && dir == "prev"){
    		...
    		$(".incident-block").animate({left: default_left});
	...
	function changeSlide(direction, options){
		...
		var counter_number = options.next_slide_number + 1;
		$(".counter .current").text(counter_number == 11 ? 10 : counter_number)

		var speed = isMobile ? 200 : 300;
